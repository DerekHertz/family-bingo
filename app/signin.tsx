 ...
 *
 * ## Why this is `/signin` and no longer `/`
 *
 * `/` is the landing page now — a public address that goes on a CV and has to say what this
 * is to somebody with no Account and no way of getting one (sign-up is invite-only,
 * `20260801000037`). This screen kept the redirect that used to live on `/`, and that is the
 * right home for it: the rule was never "bounce anyone signed in off the front page", it was
 * **never show a sign-in form to somebody who is already signed in**, which is a fact about
 * this screen. Landing keeps its own address for a signed-in Member and simply changes what
 * its first button says.
 */
